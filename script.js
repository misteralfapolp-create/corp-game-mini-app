...
(function createStars(){
    ...
    for(var i=0;i<120;i++){
        ...
        star.style.cssText='left:'+Math.random()*100+'%;top:'+Math.random()*100+'%;width:'+Math.random()*3+'px;height:'+Math.random()*3+'px;--dur:'+(2+Math.random()*4)+'s;animation-delay:'+Math.random()*4+'s';
...
async function updateAllStats(){
    ...
    if(currentUser.owner_id&&currentUser.owner_id!==currentUser.vk_id){
        ...
        quitBtn.onclick=async function(){
            if(currentUser.experience<myCost){toast('Недостаточно опыта!','error');return}
            ...
            currentUser.experience-=myCost;currentUser.owner_id=null;currentUser.status='Биржа труда';
...
async function upgradeEmployee(emp){
    ...
    if(currentUser.experience<cost){toast('Недостаточно опыта!','error');return}
    ...
    currentUser.experience-=cost;toast('✅ Прокачано!','success');await updateAllStats();loadMyTeam(true);renderAll();
}
...
async function fireEmployee(emp){
    ...
    currentUser.experience+=fireIncome;toast('🔥 Уволен! +'+fireIncome+' опыта','info');await updateAllStats();loadMyTeam(true);renderAll();
}
...
async function loadClansInTopScreen(){
    var c=document.getElementById('top-content');c.innerHTML='Загрузка...';
    ...
    c.innerHTML='';if(!sorted.length){c.innerHTML='<p style="color:#aaa;">Кланов нет</p>'}
...
// ================= ЭКРАН КЛАНОВ =================
async function loadClansScreen(){
    var c=document.getElementById('clans-content');c.innerHTML='Загрузка...';
    var r=await supabase.from('players').select('company').neq('company',null);
    if(r.error){c.innerHTML='Ошибка';return}
    var clans={};r.data.forEach(function(p){if(!p.company)return;if(!clans[p.company])clans[p.company]={name:p.company,count:0};clans[p.company].count++});
    var sorted=Object.values(clans).sort(function(a,b){return b.count-a.count});
    c.innerHTML='';if(!sorted.length){c.innerHTML='<p style="color:#aaa;">Кланов пока нет</p>'}
    sorted.forEach(function(cl,i){
        var isMine=cl.name===currentUser.company;
        var div=document.createElement('div');div.className='player-item';div.style.background=isMine?'rgba(76,175,80,0.1)':'';
        div.innerHTML='<div style="font-weight:700;width:25px;">'+(i+1)+'.</div><div class="info"><div class="name">'+cl.name+(isMine?' ⭐':'')+'</div><div class="detail">👥 '+cl.count+' участников</div></div>';
...
function openPlayerModal(player){
    ...
    // Нельзя нанять своего владельца
    var isMyOwner=currentUser.owner_id&&currentUser.owner_id===player.vk_id;
    if(player.status==='Биржа труда'&&player.vk_id!==currentUser.vk_id&&!isMyOwner){
...
async function hirePlayer(empId){
    ...
    currentUser.experience-=100;toast('✅ Нанят!','success');await updateAllStats();loadMyTeam(true);renderAll();
}
    ...
    if(r.data){
        ...
        if(currentUser.company===name){
            ...
            lb.onclick=async function(){if(currentUser.experience<cost){toast('Недостаточно опыта!','error');return}
                ...
                currentUser.experience-=cost;currentUser.company=null;toast('Вышли из клана','info');closeClanModal();location.reload()};
