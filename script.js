...
// ================= АВТООБНОВЛЕНИЕ КЭША =================
// Добавляем мета-тег для запрета кэширования
(function(){
...
async function initApp(){
    try{
        ...
        if(!r.data){
            ...
            // Если есть реферал — привязываем к владельцу
            if(invitedBy){
                var inviter=await supabase.from('players').select('company,status,vk_id').eq('vk_id',parseInt(invitedBy)).maybeSingle();
                if(inviter.data){
                    // Назначаем владельца
        ...
        // Обработка реферала для существующего игрока
...
function doGroupTask(){
    ...
    toast('📱 Открыта группа. Подпишитесь и нажмите «Проверить»','info');
}

async function checkGroupTask(){
    if(currentUser.task_group_done){toast('Задание уже выполнено!','info');return}
    // Пытаемся проверить подписку через VK API
    try{
        var groupId=240295160;
        var result=await vkBridge.send('VKWebAppCallAPIMethod',{
            method:'groups.isMember',
            params:{group_id:groupId,user_id:currentUser.vk_id,v:'5.199'}
        });
        if(result.response===1){
            ...
            toast('✅ +1000 опыта за подписку!','success');renderAll();
        }else{toast('Вы не подписаны на группу','error')}
    }catch(e){
        // Если API не сработал — всё равно даём награду
        await supabase.from('players').update({experience:(currentUser.experience||0)+1000,task_group_done:true}).eq('vk_id',currentUser.vk_id);
        currentUser.experience+=1000;currentUser.task_group_done=true;
...
function doPromoTask(){openSettings();toast('Введите любой промокод','info')}
...
async function applyPromo(){
    ...
    // Если задание с промокодом ещё не выполнено — засчитываем
    if(!currentUser.task_promo_done){await supabase.from('players').update({experience:currentUser.experience+1000,task_promo_done:true}).eq('vk_id',currentUser.vk_id);currentUser.experience+=1000;currentUser.task_promo_done=true;toast('🎁 +'+promo.reward_exp+' опыта + бонус 1000 за задание!','success')}
...
function renderTasks(){
    ...
    var html='';
    // Задание 1: Подписка на группу
    html+='<div class="task-item">';
    html+='<div class="task-info"><b>📱 Подписаться на группу</b><br><span style="font-size:11px;color:#aaa;">Награда: 1000 опыта</span></div>';
    ...
    // Задание 2: Промокод
    html+='<div class="task-item">';
    html+='<div class="task-info"><b>🎁 Ввести промокод</b><br><span style="font-size:11px;color:#aaa;">Награда: 1000 опыта</span></div>';
