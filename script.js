...
(function(){var c=document.getElementById('stars-canvas');for(var i=0;i<80;i++){var s=document.createElement('div');s.className='star';s.style.cssText='left:'+Math.random()*100+'%;top:'+Math.random()*100+'%;width:'+Math.random()*2+'px;height:'+Math.random()*2+'px;--dur:'+(2+Math.random()*4)+'s;animation-delay:'+Math.random()*4+'s';c.appendChild(s)}})();
...
async function initApp(){
    try{
        ...
        if(!r.data){
            ...
            if(invitedBy){var inviter=await supabase.from('players').select('vk_id').eq('vk_id',parseInt(invitedBy)).maybeSingle();if(inviter.data){await supabase.from('players').update({owner_id:parseInt(invitedBy),status:'Работает',role:'Учёный',income_per_hour:1}).eq('vk_id',currentVkUser.id);await giveReferralBonus(parseInt(invitedBy))}}
...
async function updateAllStats(){
    ...
    if(myTeam.length>0){
        var myIds=myTeam.map(function(e){return e.vk_id});
        var subResult=await supabase.from('players').select('income_per_hour').in('owner_id',myIds);
...
async function loadTopPlayersScreen(){
    ...
    var allResult=await supabase.from('players').select('vk_id,first_name,last_name,photo_200,experience,owner_id').order('experience',{ascending:false}).limit(200);
    if(allResult.error){c.innerHTML='Ошибка';return}
    var allIds=allResult.data.map(function(p){return p.vk_id});
    var counts={};
    if(allIds.length>0){var countResult=await supabase.from('players').select('owner_id').in('owner_id',allIds);if(countResult.data){countResult.data.forEach(function(r){counts[r.owner_id]=(counts[r.owner_id]||0)+1})}}
    var playersWithCount=allResult.data.map(function(p){return {vk_id:p.vk_id,first_name:p.first_name,last_name:p.last_name,photo_200:p.photo_200,experience:p.experience||0,empCount:counts[p.vk_id]||0}});
